...
const TIB_EXPECTED_RUNTIME_FINGERPRINT = "ps-tib-0.18.2-20260914-a";
...
setInterval(() => chrome.runtime.sendMessage({ type: "TELEMETRY_ACTIVITY" }).catch(() => {}), 60_000);
document.addEventListener("click", (event) => {
  const tab = event.target.closest("[data-tab]");
  if (!tab) return;
  const label = String(tab.textContent || "").trim();
  if (label.startsWith("MVP")) chrome.runtime.sendMessage({ type: "TELEMETRY_MVP", mvp: label }).catch(() => {});
});
...
function feedbackClassName(message = "", tone = "") {
  const loading = !tone && /^(đang|loading|reading|processing|uploading)\b/i.test(String(message).trim());
  return `upload-feedback${message ? " show" : ""}${tone ? ` ${tone}` : loading ? " loading" : ""}`;
}

function setUploadFeedback(message = "", tone = "") {
  const feedback = $("#upload-feedback");
  feedback.textContent = message;
  feedback.className = feedbackClassName(message, tone);
}
...
async function saveLarkSettings() {
  ...
  try {
    ...
    setLarkApiStatus("online", "API đang online");
    showToast("Đã lưu và kết nối Lookup API.");
  } catch (error) {
    if (isApiOfflineError(error)) {
      setLarkApiStatus("offline", "API offline · kiểm tra server port 8787");
    }
...
function setLarkApiStatus(status, message) {
  const element = $("#lark-api-status");
  const text = $("#lark-api-status-text");
  if (!element || !text) return;
  element.className = `api-status ${status || ""}`.trim();
  text.textContent = message;
}

function isApiOfflineError(error) {
  return /failed to fetch|networkerror|network request failed|load failed|fetch failed|timed out|timeout|connection refused|actively refused/i.test(
...
function renderLarkLookup(result) {
  ...
  const giftbox = result.giftbox || {};
  setLookupField("lookup-giftbox-handle", giftbox.productHandle || "");
  $("#lookup-giftbox-source").textContent = giftbox.found
    ? `Source: Giftbox Sheet, row ${giftbox.sourceRows.join(", ")}${giftbox.ambiguous ? " · Có dữ liệu xung đột" : ""}`
    : "Không tìm thấy Product Type trong Giftbox Sheet.";

  const feedback = $("#lark-feedback");
  if (shipping.ambiguous || gmc.ambiguous || giftbox.ambiguous) {
  ...
  } else if (!shipping.found && !gmc.found && !giftbox.found) {
...
async function fillLookupIntoShopify(result = state.larkLookup) {
  ...
  if (result.shipping?.ambiguous || result.gmc?.ambiguous || result.giftbox?.ambiguous) {
...
async function fillLookupIntoShopifyStable(result = state.larkLookup) {
  ...
  const expected = SHOPIFY_LARK_FIELDS
    ...
    .map(({ label }) => label);
  if (String(result.giftbox?.productHandle || "").trim()) expected.push("Giftbox");
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    if (result.giftbox?.productHandle) {
      try {
        giftboxResponse = await chrome.runtime.sendMessage({
          type: "FILL_SHOPIFY_GIFTBOX",
          tabId: activeTabId,
          productHandle: result.giftbox.productHandle
        });
        if (giftboxResponse?.error) lastError = new Error(giftboxResponse.error);
      } catch (error) {
        lastError = error;
      }
    }
    ...
    response = {
      ...
      giftboxDiagnostics: giftboxResponse?.diagnostics || giftboxResponse?.stage || "not-run",
      attempts: attempt
    };
...
async function lookupLatestLarkData() {
  ...
  feedback.className = "upload-feedback show loading";
  setLarkApiStatus("checking", "Đang kiểm tra Lookup API…");
  ...
  try {
    ...
    setLarkApiStatus("online", "API đang online · dữ liệu đã cập nhật");
    renderLarkLookup(body.data);
    await fillLookupIntoShopifyStable(body.data);
  } catch (error) {
    if (isApiOfflineError(error)) {
      setLarkApiStatus("offline", "API offline · kiểm tra server port 8787 rồi thử lại");
      feedback.textContent = "Không kết nối được Lookup API. Hãy khởi động server port 8787 rồi bấm lại.";
    } else {
      feedback.textContent = error.message;
      if (/api|lark|fetch/i.test(String(error?.message || ""))) {
        setLarkApiStatus("offline", "API không phản hồi đúng · kiểm tra cấu hình/API key");
      }
    }
    feedback.className = "upload-feedback show error";
  } finally {
...
async function uploadMockupsToShopify() {
  ...
  feedback.className = feedbackClassName(feedback.textContent);
  ...
  return files
    .map((file) => {
      ...
      const nameMatch = fileName.match(
        /^mk-(?:(\d+)(?:-(default|df|detail|review))?|(default|df|detail|review))\.(png|jpe?g|webp)$/i
      );
      if (!nameMatch) return null;
      const numbered = Boolean(nameMatch[1]);
      const suffix = (nameMatch[2] || nameMatch[3] || "").toLowerCase();
      ...
      return {
        file,
        number: numbered ? Number(nameMatch[1]) : null,
        kind: numbered ? suffix || "number" : suffix,
        sortGroup: suffix === "default" || suffix === "df" ? 0 : numbered ? 1 : 2,
        campaignFolder,
...
function resolveCampaignMockups(title) {
  ...
  if (!entries.length) {
    throw new Error("Không tìm thấy MK-số, MK-số-DF/Default/Detail/Review hoặc MK-Default/MK-DF/MK-Detail/MK-Review trong folder MK / Mock up / Mockup.");
  }
...
function renderCampaignMockupSummary(selection = state.campaignMockupSelection) {
  ...
  summary.textContent = state.campaignMockupFiles.length
    ...
    : "Lấy MK-1, MK-1-DF…, MK-Default/MK-DF, MK-Detail và MK-Review trong folder MK / Mock up / Mockup";
}
...
function acceptCampaignMockupFiles(files) {
  ...
  setCampaignFeedback(
    state.campaignMockupFiles.length
      ? `Đã đọc ${state.campaignMockupFiles.length} file trong folder. Tool sẽ lấy MK-số, MK-số-DF/Default/Detail/Review và MK-Default/MK-DF đúng Record ID.`
      : "",
...
function tibCampaignMockups(files = state.tibCampaignMockupFiles) {
  return [...files]
    .filter((file) => {
      ...
      return /^mk-(?:\d+(?:-(?:default|df|detail|review))?|default|df|detail|review)$/i.test(stem) && !/(?:ads|pre)/i.test(stem);
    })
...
function renderTibCampaignMockups() {
  ...
  $("#tib-campaign-mockup-summary").textContent = files.length
    ...
    : "Lấy MK-1, MK-1-DF…, MK-Default/MK-DF, MK-Detail và MK-Review; bỏ ADS/Pre";
}
