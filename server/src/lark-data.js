...
export function lookupGiftbox(values = [], productType) {
  const matches = nonEmptyRows(values || []).filter(
    ({ sheetRow, row }) => sheetRow > 1 && rowMatchesProductType(row[0], productType)
  );
  const uniqueness = uniqueRelevantMatches(matches, [1]);
  if (!matches.length) return { found: false, ambiguous: false, sourceRows: [] };
  const selected = matches[0];
  return {
    found: true,
    ambiguous: uniqueness.ambiguous,
    sourceRows: matches.map(({ sheetRow }) => sheetRow),
    productType: cellText(selected.row[0]).trim(),
    productHandle: cellText(selected.row[1]).trim()
  };
}
...
export function buildLookupResult({
  ...
  return {
    ...
    giftbox: lookupGiftbox(giftboxValues, productType)
  };
}
