...
assert.match(
  teeinblueContentScript,
  /TIB_RUNTIME_FINGERPRINT = "ps-tib-0\.16\.19-20260819-a"/
);
assert.match(
  teeinblueContentScript,
...
assert.match(
  sidepanelScript,
/TIB_EXPECTED_RUNTIME_FINGERPRINT = "ps-tib-0\.16\.19-20260819-a"[\s\S]*?response\.runtimeFingerprint === TIB_EXPECTED_RUNTIME_FINGERPRINT/
);
...
// MVP5 mockup filter accepts numbered, Default, Detail and Review files only.
const campaignMockupNamePattern = /^mk-(\d+|default|df|detail|review)\.(png|jpe?g|webp)$/i;
for (const name of [
  "MK-1.jpg",
  "MK-002.png",
  "MK-Default.webp",
...
assert.match(
  sidepanelScript,
  /\^mk-\(\\d\+\|default\|df\|detail\|review\)[\s\S]*?sortGroup: suffix === "default" \|\| suffix === "df" \? 0 : numbered \? 1 : 2/
);
